...
$file_name = 'paradigma';
$_ROOT_PATH = 'ganymede';
$file_path = $_ROOT_PATH . '/grammar/' . $file_name . '.txt';
...
function sql_key_value_pair( $key, $value ) {

	return $key . ' = ' . sql_quateB( $value );

}

function sql_where( $key, $value ) {

	$result = 'WHERE '
		.	sql_key_value_pair( $key, $value )
		.	';';
...
function sql_update( $table_name, $assoc_array, $condition = ';' ) {

	$pairs = Array();
	foreach( $assoc_array as $key => $value ) {
		
		array_push( $pairs, sql_key_value_pair( $key, $value ) );
...
function left_near_element( $array, $index ) {
...
//echo sql_update( 'dictionary2', Array( 'a' => 1 ), sql_where( 'b', 2 ) );
...
//$output_file = fopen( $_ROOT_PATH . '/sql/' . $file_name . '-update.sql', 'w+' );
...
while( !feof( $file ) ) {
	...
	if( $current_shift != $shift ) {
		...
		if( $current_shift < $shift ) {
			...
			$_PATH = array_slice( $_PATH, 0, $current_shift );
			$_EXTREME_LINES = array_slice( $_EXTREME_LINES, 0, $current_shift + 1 );
			$token[ 'base_id' ] = left_near_element( $_EXTREME_LINES, $current_shift );
			$token[ 'path' ] = implode('.', $_EXTREME_LINES );
			$_EXTREME_LINES[ $current_shift ] = $token[ 'id' ];

		} else {

			//$_EXTREME_LINES = array_range( $_EXTREME_LINES, $shift, $current_shift, $_EXTREME_LINES[ $shift ] );
			$token[ 'base_id' ] = left_near_element( $_EXTREME_LINES, $shift );
			$token[ 'path' ] = implode('.', $_EXTREME_LINES );
	...
	$_PATH[ $current_shift ] = $token['id'];

	//print_r( $_EXTREME_LINES );

	//echo $token['word'] . '&#9;' . implode( '.', $_EXTREME_LINES ) . '&#10;';
	...
	if( empty( $_ROOT ) ) {
		...
		$_ROOT = Array( -1 => $token[ 'id' ] );
		$_PATH = Array(); // First main word
	...
	$token[ 'morph' ] = $morph;
	$token[ 'path' ] = implode( '.', $_ROOT + $_PATH );

	if( strlen( $token[ 'path' ] ) > $_MAX_PATH_SIZE ) {

		$_MAX_PATH_SIZE = strlen( $token[ 'path' ] );
		$_MAX_PATH = $token[ 'path' ];

	}

	//echo $current_shift . ' ' . $fields[ 0 ] . '&#9;' . trim( $token[ 'id' ] ) . '&#9;' . $token[ 'path' ] . '&#10;';

	$query = sql_update( 'dictionary2', Array( 'path' => $token[ 'path' ] ), sql_where( 'id', $token[ 'id' ] ) );

	//echo $fields[ 0 ] . '&#9;' . $token[ 'id' ] . '&#9;' . $token[ 'path' ] . '&#10;';
